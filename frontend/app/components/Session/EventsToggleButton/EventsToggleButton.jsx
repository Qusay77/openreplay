...
function EventsToggleButton({ showEvents, toggleEvents }) {
  ...
  return (
    <Popup2
      trigger={
        <button className={cn('absolute right-0 z-50', stl.wrapper)} onClick={toggle}>
          <Icon name={showEvents ? 'chevron-double-right' : 'chevron-double-left'} size="12" />
        </button>
      }
