...
const reducer = (state = initialState, action = {}) => {
  switch (action.type) {
    ...
    //   }
    //   return state
    //     .set('list', List(action.data.map(Site)))
    //     .set('siteId', siteId)
    //     .set(
    //       'active',
    //       List(action.data.map(Site)).find((s) => s.id === parseInt(siteId))
    //     );
    case FETCH_LIST_SUCCESS:
      const projectId = window.location.pathname.split('/')[1];
      const HC = [{ name: 'wz', projectId, recorded: true }];
      let siteId = HC.projectId;

      return state
        .set('list', List(HC.map(Site)))
        .set('siteId', siteId)
        .set(
          'active',
          List(HC.map(Site)).find((s) => s.id === parseInt(siteId))
        );
    case SET_SITE_ID:
