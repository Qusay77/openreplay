...
const reducer = (state = initialState, action = {}) => {
  switch (action.type) {
    ...
    case FETCH_ERROR_STACK.SUCCESS:
      return state
        .set('errorStack', List(action.data.trace).map(ErrorStack))
    ...
    case FETCH_LIST.SUCCESS:
      ...
      const eventProperties = {
        ...
        filterKeys: Object.keys(params).filter(
          (key) =>
            !['custom', 'startDate', 'endDate', 'strict', 'key', 'events', 'rangeValue'].includes(
              key
            )
        ),
      ...
      let filteredEvents = query
        ? events.filter(
            (e) =>
              ...
              (e.payload &&
                (searchRe.test(e.payload.title) ||
                  searchRe.test(e.payload.details.error) ||
                  searchRe.test(e.payload.details.ajax_status_code) ||
                  searchRe.test(msToTime(e.payload.details.duration))))
          )
        : null;
    ...
    case FETCH.SUCCESS: {
      ...
      const current =
        state.get('list').find(({ sessionId }) => sessionId === action.data.sessionId) || Session();
    ...
    case TOGGLE_FAVORITE.SUCCESS: {
      ...
      return state
        .update('list', (list) =>
          list.map((session) =>
            session.sessionId === id ? session.set('favorite', !wasInFavorite) : session
          )
        )
    ...
    case REDEFINE_TARGET: {
      ...
      return state.updateIn(['current', 'events'], (list) =>
        list.map((event) =>
          event.target && event.target.path === path
    ...
    case LAST_PLAYED_SESSION_ID:
      return updateListItem(state, action.sessionId, { viewed: true }).set(
        'lastPlayedSessionId',
        action.sessionId
      );
    default:
...
export const fetch =
  ...
  (dispatch, getState) => {
    dispatch({
      types: FETCH.toArray(),
      call: (client) =>
        client.get(isLive ? `/assist/sessions/${sessionId}` : `/sessions2/${sessionId}`),
