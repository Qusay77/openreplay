...
export default Record(
  ...
  },
  {
    fromJS: ({
      ...
      const eventsContact = isWzSession
        ? [].concat(session.events).concat(session.userEvents)
        : session.events;

      const eventsSorted = eventsContact && eventsContact.sort((a, b) => a.timestamp - b.timestamp);

      const events = List(eventsSorted)
        .map((e) => SessionEvent({ ...e, time: (e && e.timestamp) - startedAt }))
      ...
      return {
        ...
        userNumericHash: hashString(
          session.userId || session.userAnonymousId || session.userUuid || ''
        ),
